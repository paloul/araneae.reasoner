package paloul.araneae.reasoner.util
...
trait PerformanceMetrics {
   ...
   */
  def time[R](name:String, block: => R)(implicit log: Logger): R = {
    val t0 = System.nanoTime()
    val result = block // call-by-name, will execute func block
    val t1 = System.nanoTime()

    log.info(s"[$name] Elapsed time: ${(t1 - t0) / 1E9} (s)") // Print out in seconds (nano / 1E9)
