...
object UUID5 {
   ...
   * @param name Any arbitrary string value
   * @return A UUID5 always unique to the given name
   */
  def apply(name: String): UUID5 = {
   ...
   * @return UUID5
   */
  private def generateType5UUID(namespace: String, name: String): UUID5 = {
   ...
   * @return
   */
  private def type5UUIDFromBytes(name: Array[Byte]): UUID5 = {
    import java.util
   ...
   */
  private def constructType5UUID(data: Array[Byte]): UUID5 = {
    var msb = 0
    var lsb = 0
...
class UUID5 private (val mostSigBits: Long, val leastSigBits: Long) extends UUID {
  ...
  private val uuid: java.util.UUID = new java.util.UUID(mostSigBits, leastSigBits)
}
